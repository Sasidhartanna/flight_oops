...
public class Admin extends User {
	...
	private static final long serialVersionUID = 1L;
       public boolean add_flight(HttpServletRequest request,HttpServletResponse response) {
    	   ...
    	   String sql="insert into flight.airplane(company_id,route_id,start_time,dest_time,seat_eco,seat_bus,price1,domestic,mon,tue,wed,thu,fri,sat,sun,price2,mon_seats,tue_seats,wed_seats,thu_seats,fri_seats,sat_seats,sun_seats,mon_seats_bus,tue_seats_bus,wed_seats_bus,thu_seats_bus,fri_seats_bus,sat_seats_bus,sun_seats_bus) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
    	   String sql1;
    	   if(dom==1)
    	   {
    		  sql1="update  flight.international set international.domestic=1 where place IN(select from1 from route where id=?  union select to1 from route where id=?);";
    	   }
    	   else
    	   {
     		  sql1="update  flight.international set international.international=1  where place IN(select from1 from route where id=?  union select to1 from route where id=?);";
    	   }
       ...
       public boolean add_route(HttpServletRequest request, HttpServletResponse response) {
    	   String from1=request.getParameter("from1").toString();
    	   String to1=request.getParameter("to1").toString();
    	   float distance=Float.parseFloat(request.getParameter("dist").toString());
    	   String sql="insert into flight.route(distance,from1,to1) values(?,?,?) ";
    	   String sql1="insert into flight.international(place) values(?)";
    	   String sql2="select place from international where place=(?)";
    	   ...
    	   try {
    		   String url="jdbc:mysql://localhost:3306/flight?useSSL=false";
        		...
        		PreparedStatement st=con.prepareStatement(sql);
        		st.setFloat(1, distance);
        		st.setString(2, from1);
        		st.setString(3, to1);
        		int x=st.executeUpdate();
        		if(x>0) {
        			return true;
